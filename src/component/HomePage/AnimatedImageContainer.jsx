...
const AnimatedImageContainer = () => {
  ...
  return (
    <div
      className="relative w-full h-48 sm:h-64 md:h-80 lg:h-96 bg-cover bg-center transition-opacity duration-300 py-10 bg-[#10375C] hidden sm:block" // Responsive height
      style={{ backgroundImage: "url('../../images/background.jpg')" }}
    >
      <div className="absolute inset-0 flex flex-col sm:flex-row justify-between items-center p-4">
        {/* Image 1 */}
        <div className={`w-full sm:w-1/5 h-auto ${activeIndex === 0 ? 'scale' : ''} mx-2`}>
          <img src={img1} alt="Image 1" className="image w-full h-full object-contain" />
        </div>

        {/* Image 2 */}
        <div className={`w-full sm:w-1/5 h-auto ${activeIndex === 1 ? 'scale' : ''} mx-2`}>
          <img src={img2} alt="Image 2" className="image w-full h-full object-contain" />
        </div>

        {/* Image 3 */}
        <div className={`w-full sm:w-1/5 h-auto ${activeIndex === 2 ? 'scale' : ''} mx-2`}>
          <img src={img3} alt="Image 3" className="image w-full h-full object-contain" />
        </div>

        {/* Image 4 */}
        <div className={`w-full sm:w-1/5 h-auto ${activeIndex === 3 ? 'scale' : ''} mx-2`}>
          <img src={img4} alt="Image 4" className="image w-full h-full object-contain" />
        </div>

        {/* Image 5 */}
        <div className={`w-full sm:w-1/5 h-auto ${activeIndex === 4 ? 'scale' : ''} mx-2`}>
