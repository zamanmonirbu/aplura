...
const RightSide = () => {
  ...
  return (
    <div className="fixed top-1/3 right-2 flex flex-col space-y-4 hidden md:flex z-50">
      {menuItems.map((item, index) => (
        ...
        >
          {/* Icon */}
          <div
            className="bg-black text-white p-3 rounded-full cursor-pointer z-50"
            onClick={() => handleClick(index)} // Toggle onClick
          >
            {item.icon}
          </div>

          {/* Hover Content (Now shown on click) */}
          <div
            className={`absolute right-14 top-1/2 transform -translate-y-1/2 
            ${activeIndex === index ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}
            bg-white text-black rounded-lg shadow-md px-4 py-2 transition-all duration-300 flex items-center z-50`}
          >
